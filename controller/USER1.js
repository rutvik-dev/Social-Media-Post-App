...
let SignupUser = async function (req, res, next) {
    try {
        ...
        const CreateUser = await User.create({ username, email, password: hashedPassword, posts });
...
let ReadAllUser = async function (req, res, next) {
    try {
        ...
        const ReadUsers = await User.find().populate('posts');
        res.status(201).json({
            status: "success",
...
let UpdateUser = async function (req, res, next) {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(id, req.body, { new: true }).populate('posts');
        if (!updatedUser) {
            return res.status(404).json({ message: 'User not found' });
        }
        res.status(201).json({
